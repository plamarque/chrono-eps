...
| **Tableau relais** | Affichage « Couru » / « Prochain » par groupe, tap par groupe ; modale groupe `RelayGroupModal.vue` ; pas d’édition groupe tant que `participantStates[group].status === 'running'` ; drapeau passage zone tactile élargie | src/components/TableauPassagesRelay.vue |
| **Cartes individuel (terrain + fiche)** | Grille type relais : en-tête couleur, Play/Stop, drapeau tour, liste des tours et total ; ajout via **Coureur** (HomeView : `handleAddCoureur` — `recordPassage` + `pauseParticipantAtRecordedTotal` sur le dernier en course, puis nouveau coureur synchronisé si chrono lancé ; sinon ajout au temps du clic) ; fiche historique : `readOnly` + `allowParticipantEdit` pour le nom | src/components/TableauPassagesIndividualCards.vue |

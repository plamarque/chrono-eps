...
| **Cartes individuel (terrain + fiche)** | Grille type relais : en-tête couleur, Play/Stop, drapeau tour, liste des tours et total ; modale nom/couleur **uniquement** depuis l’en-tête ; pas d’édition tant que `participantStates[id].status === 'running'` (terrain) ; ajout via **Coureur** (HomeView : `handleAddCoureur` — en course, ajoute un coureur synchronisé `running` sans arrêter les coureurs déjà en course ; hors course, ajout au temps du clic) ; fiche historique : `readOnly` + `allowParticipantEdit` pour le nom | src/components/TableauPassagesIndividualCards.vue |

...
describe('useChronometre mode individuel (états par coureur)', () => {
  beforeEach(() => {
    vi.useFakeTimers()
  })
  ...
  it('sans participants : start ne change pas le statut (idle)', () => {
    const participants = ref([])
    const chrono = useChronometre(participants, { mode: 'individual' })
    chrono.start()
    expect(chrono.status.value).toBe('idle')
    expect(chrono.elapsedMs.value).toBe(0)
  })

  it('individuel : seed join puis premier tour = delta depuis join', async () => {
    const participants = ref([{ id: 'c1', nom: 'Coureur 1', color: '#ef4444' }])
    const chrono = useChronometre(participants, { mode: 'individual' })
    chrono.seedIndividualParticipantAtJoin('c1', 8000)
    const wrapper = mount({
      setup: () => () =>
        h('div', [
          h('span', { 'data-testid': 'passages' }, JSON.stringify(chrono.passagesByParticipant.value)),
          h('button', { onClick: chrono.start }, 'Start'),
          h('button', { onClick: () => chrono.recordPassage('c1') }, 'Record')
        ])
    })
    await wrapper.findAll('button')[0].trigger('click')
    await vi.advanceTimersByTimeAsync(2000)
    ...
    expect(passages.c1[0].lapMs).toBeGreaterThanOrEqual(1900)
    expect(passages.c1[0].lapMs).toBeLessThanOrEqual(2100)
    expect(passages.c1[0].totalMs).toBeGreaterThanOrEqual(9900)
    expect(passages.c1[0].totalMs).toBeLessThanOrEqual(10100)
    wrapper.unmount()
  })

  it('individuel : firstLapFromRaceStart mesure le tour 1 depuis le départ course (0 ms)', async () => {
    const participants = ref([{ id: 'c1', nom: 'Coureur 1', color: '#ef4444' }])
    const chrono = useChronometre(participants, { mode: 'individual' })
    chrono.start()
    await vi.advanceTimersByTimeAsync(5000)
    ...
    await nextTick()
    const atJoin = chrono.elapsedMs.value
    expect(atJoin).toBeGreaterThanOrEqual(4900)
    expect(atJoin).toBeLessThanOrEqual(5100)
    chrono.seedIndividualParticipantAtJoin('c2', atJoin, true)
    chrono.startParticipant('c2')
    await vi.advanceTimersByTimeAsync(2000)
    chrono.recordPassage('c2')
    const p2 = chrono.passagesByParticipant.value.c2
    expect(p2).toHaveLength(1)
    expect(p2[0].lapMs).toBeGreaterThanOrEqual(6900)
    expect(p2[0].lapMs).toBeLessThanOrEqual(7100)
    expect(p2[0].totalMs).toBeGreaterThanOrEqual(6900)
    expect(p2[0].totalMs).toBeLessThanOrEqual(7100)
  })

  it('recordPassage enregistre un tour sans studentIndex', async () => {
    const participants = ref([{ id: 'c1', nom: 'Coureur 1', color: '#ef4444' }])
    const chrono = useChronometre(participants, { mode: 'individual' })
    const wrapper = mount({
      setup: () => () =>
    ...
    await wrapper.vm.$nextTick()
    const passages = JSON.parse(wrapper.find('[data-testid="passages"]').text())
    expect(passages.c1).toHaveLength(2)
    expect(passages.c1[0]).not.toHaveProperty('studentIndex')
    expect(passages.c1[1]).not.toHaveProperty('studentIndex')
    expect(passages.c1[1].totalMs).toBeGreaterThan(passages.c1[0].totalMs)
    wrapper.unmount()
  })
  ...
  it('stopParticipant enregistre un passage (individuel)', async () => {
    const participants = ref([{ id: 'c1', nom: 'Coureur 1', color: '#ef4444' }])
    const chrono = useChronometre(participants, { mode: 'individual' })
    const wrapper = mount({
      setup: () => () =>
        h('div', [
          h('span', { 'data-testid': 'passages' }, JSON.stringify(chrono.passagesByParticipant.value)),
          h('button', { onClick: chrono.start }, 'Start'),
          h('button', { onClick: () => chrono.stopParticipant('c1') }, 'StopP1')
        ])
    })
    await wrapper.findAll('button')[0].trigger('click')
